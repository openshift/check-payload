...
Version-specific configurations are embedded from `dist/releases/4.x/config.toml` during build. `fips_certified_modules` lives in root `config.toml`; version configs handle exceptions only.

## FIPS Module Validation

Each `fips_certified_modules` entry maps a crypto stack (`module`) to its `certified_artifact`, with optional `certified_artifact_min_version` and `certified_artifact_paths` for file-based fallback. Module validation runs when entries are present. Artifact presence verified via RPM query or file path. Host library FIPS checks are code-driven via `moduleHostLibChecks` in `validations_host_crypto.go`.

Flow: OS allowlist check runs early. After binary scanning, detected crypto modules are matched against config entries and each artifact is validated.
